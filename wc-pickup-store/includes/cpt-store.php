...
function wps_check_countries_count($only_validate = true) {
	if ( ! wps_is_country_filtering_disabled() ) {
		...
		if ( is_array( $specific_allowed_countries ) && count( $specific_allowed_countries ) > 1 ) {
			if ( $only_validate ) {
				return true;
			}
...
/**
 * Disable CPT for gutenberg builder
 * 
 * @version 1.8.7
 */
function wps_disable_gutenberg_for_stores( $use_block_editor, $post_type ) {
	if ( $post_type === 'store' ) {
		return false;
	}
...
add_filter( 'use_block_editor_for_post_type', 'wps_disable_gutenberg_for_stores', 10, 2 );
